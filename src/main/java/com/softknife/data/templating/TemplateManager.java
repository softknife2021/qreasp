/*
 * *
 *  * Created by SOFTKNIFE on 8/7/21, 2:01 PM
...
public class TemplateManager {
    ...
    public String processTemplate(String templateName, String version, Map<String, Object> data) throws RecordNotFound, IOException, TemplateException {
        if (StringUtils.isBlank(templateName) || StringUtils.isBlank(version)) {
            // Either blank is enough. With `&&` a blank version got through and failed later as
            // "not able to find template", which sends the reader looking for a missing file.
            throw new IllegalArgumentException("Both templateName and version are required, got name='"
    ...
    private void validateTemplateAndVersion(String templateName, String version) {
        if (StringUtils.isBlank(templateName) || StringUtils.isBlank(version)) {
            // Either blank is enough. With `&&` a blank version got through and failed later as
            // "not able to find template", which sends the reader looking for a missing file.
            throw new IllegalArgumentException("Both templateName and version are required, got name='"
    ...
    private TemplateHolder findTemplate(String name, String version) throws RecordNotFound {
        Optional<TemplateHolder> optional = this.templateHolderList.stream()
